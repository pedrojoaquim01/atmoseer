...
def aggregation(f, n, inic, fim):
    ...
    for s in result:
        ...
        if s in cor_est:
            ...
            df1 = df1.drop(columns=['Dia','Hora','estacao','HBV','Chuva','DirVento','VelVento','Temperatura','Pressao','Umidade','data'])
            
            train_df1 = train_df1.drop(columns=['Dia','Hora','estacao','HBV','data'])
            val_df1 = val_df1.drop(columns=['Dia','Hora','estacao','HBV','data'])
            test_df1 =  test_df1.drop(columns=['Dia','Hora','estacao','HBV','data'])

        else:
            df1 = df1.drop(columns=['DC_NOME','UF','DT_MEDICAO','CD_ESTACAO','VL_LATITUDE','VL_LONGITUDE','HR_MEDICAO','data','TEM_SEN','PRE_MAX','RAD_GLO','PTO_INS','TEM_MIN','UMD_MIN','PTO_MAX','PRE_MIN','UMD_MAX','PTO_MIN','TEM_MAX','TEN_BAT','VEN_RAJ','TEM_CPU'])
    
            train_df1 = train_df1.drop(columns=['DC_NOME','UF','DT_MEDICAO','CD_ESTACAO','VL_LATITUDE','VL_LONGITUDE','HR_MEDICAO','data','TEM_SEN','PRE_MAX','RAD_GLO','PTO_INS','TEM_MIN','UMD_MIN','PTO_MAX','PRE_MIN','UMD_MAX','PTO_MIN','TEM_MAX','TEN_BAT','VEN_RAJ','TEM_CPU'])
            val_df1 = val_df1.drop(columns=['DC_NOME','UF','DT_MEDICAO','CD_ESTACAO','VL_LATITUDE','VL_LONGITUDE','HR_MEDICAO','data','TEM_SEN','PRE_MAX','RAD_GLO','PTO_INS','TEM_MIN','UMD_MIN','PTO_MAX','PRE_MIN','UMD_MAX','PTO_MIN','TEM_MAX','TEN_BAT','VEN_RAJ','TEM_CPU'])
            test_df1 =  test_df1.drop(columns=['DC_NOME','UF','DT_MEDICAO','CD_ESTACAO','VL_LATITUDE','VL_LONGITUDE','HR_MEDICAO','data','TEM_SEN','PRE_MAX','RAD_GLO','PTO_INS','TEM_MIN','UMD_MIN','PTO_MAX','PRE_MIN','UMD_MAX','PTO_MIN','TEM_MAX','TEN_BAT','VEN_RAJ','TEM_CPU'
            ])
        
        if not df1.empty:
            df_arr = np.array(df1)
            train_arr = np.array(train_df1)
            val_arr = np.array(val_df1)
            test_arr = np.array(test_df1)

            WS = 6 # size of window to use
            IDX_TARGET = 1 # index position of the target variable
            if s in cor_est:
                IDX_TARGET = train_df1.columns.get_loc("Chuva")
            else:
                IDX_TARGET = train_df1.columns.get_loc("CHUVA")
            ...
            Train_X, Train_y = apply_windowing(train_arr, 
                                    initial_time_step=0, 
                                    max_time_step=len(train_arr)-WS-1, 
                                    window_size = WS, 
                                    idx_target = IDX_TARGET)

            if s in cor_est:
                IDX_TARGET = val_df1.columns.get_loc("Chuva")
            else:
                IDX_TARGET = val_df1.columns.get_loc("CHUVA")

            Val_X, Val_y = apply_windowing(val_arr, 
                                    initial_time_step=0, 
                                    max_time_step=len(val_arr)-WS-1, 
                                    window_size = WS, 
                                    idx_target = IDX_TARGET,
                                    only_y_not_nan = False,
                                    only_y_gt_zero = False,
            ...
            if len(Train_X) > 0:
                ...
                #df_out= pd.DataFrame()
                outfile = open('../data/Janelamento_Train_'+ s+'_X','wb')
                pickle.dump(Train_X,outfile)
                
                
                outfile = open('../data/Janelamento_Train_'+ s+'_y','wb')
                pickle.dump(Train_y,outfile)

                
                outfile = open('../data/Janelamento_Val_'+ s+'_X','wb')
                pickle.dump(Val_X,outfile)
                
                
                outfile = open('../data/Janelamento_Val_'+ s+'_y','wb')
                pickle.dump(Val_y,outfile)
